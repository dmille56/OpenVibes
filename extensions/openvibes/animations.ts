...
async function collectMilliFiles(
  root: string,
  source: 'bundled' | 'user',
): Promise<OpenVibesAnimation[]> {
  ...
  async function walk(dir: string): Promise<void> {
    ...
    for (const entry of entries) {
      ...
      if (
        !entry.isFile() ||
        path.extname(entry.name).toLowerCase() !== '.milli'
      )
        continue;
      result.push({
        name: animationNameFromFile(fullPath),
        path: fullPath,
        source,
      });
    }
...
export async function discoverOpenVibesAnimations(): Promise<
  OpenVibesAnimation[]
> {
...
export async function loadOpenVibesAnimation(
  pathToMilli: string,
): Promise<AsciiPlayer> {
