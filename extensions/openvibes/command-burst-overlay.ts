...
} satisfies Record<BurstMode, { title: RGB; subtitle: RGB; accent: RGB; haze: RGB }>;
...
function colorize(text: string, rgb: RGB, mode: "bold" | "dim" | "normal" = "normal"): string {
	const prefix = mode === "bold" ? "1;" : mode === "dim" ? "2;" : "";
	return `\x1b[${prefix}38;2;${rgb[0]};${rgb[1]};${rgb[2]}m${text}\x1b[0m`;
}

function makeLine(text: string, width: number, rgb: RGB, mode: "bold" | "dim" | "normal" = "normal"): string {
...
function placeParticle(line: string, x: number, glyph: string, color: RGB, bright: boolean): string {
...
export class CommandBurstOverlayComponent implements Component {
	...
	private buildFrame(width: number): string[] {
		...
		lines[topRow] = makeLine(makeRuneBand(cols, this.mode === "flash" ? "✧" : "⟡"), cols, palette.haze, bright ? "bold" : "dim");
		lines[titleRow] = makeLine(this.title, cols, palette.title, "bold");
		lines[subtitleRow] = makeLine(this.subtitle, cols, palette.subtitle, bright ? "normal" : "dim");
		lines[bottomRow] = makeLine(makeRuneBand(cols, "·"), cols, palette.accent, "dim");
		...
		for (const particle of this.particles) {
			...
			if (y < 0 || y >= rows || y === topRow || y === titleRow || y === subtitleRow || y === bottomRow) continue;
			const lineFade = Math.max(0, 1 - age / particle.ttl);
			lines[y] = placeParticle(lines[y] ?? " ".repeat(cols), x, particle.glyph, particle.color, lineFade > 0.5);
		}
