...
const PARTICLE_COLORS: Array<Cell['fg']> = [
  [160, 255, 208],
  [126, 216, 255],
  [215, 255, 166],
  [255, 226, 146],
...
function decoratePulse(
  ...
  const banner = `${bright}${centerText(`${accent} TOOL ${accent}`, width)}\u001B[0m`;
  const footer = `${bright}${centerText(mode === 'flash' ? 'spell cast' : 'settling', width)}\u001B[0m`;
...
export class MilliOverlayComponent implements Component {
  ...
  private spawnParticle(now: number, cols: number, rows: number): Particle {
    ...
    return {
      ...
      ttl:
        PARTICLE_TTL_MIN_MS +
        Math.random() * (PARTICLE_TTL_MAX_MS - PARTICLE_TTL_MIN_MS),
      glyph:
  ...
  private advanceParticles(now: number, cols: number, rows: number): void {
    ...
    this.particles = this.particles.filter(
      (particle) =>
        now - particle.bornAt < particle.ttl &&
        particle.y >= -1 &&
        particle.x >= -1 &&
        particle.x <= cols + 1,
    );
    ...
    while (
      ...
      this.particles.length < PARTICLE_MAX_COUNT
    ) {
  ...
  private paintParticles(
    ...
    for (const particle of this.particles) {
      ...
      const fg = particle.fg.map((channel) =>
        Math.max(0, Math.min(255, Math.round(channel * dim))),
      ) as unknown as Cell['fg'];
      row[x] = {
        ...cell,
  ...
  pulse(mode: PulseMode): void {
    this.pulseMode = mode;
    this.pulseUntil =
      Date.now() + (mode === 'flash' ? FLASH_DURATION_MS : SETTLE_DURATION_MS);
    this.tui.requestRender();
  }
  ...
  render(width: number): string[] {
    ...
    const frame = this.paintParticles(
      scaleGrid(this.player.frameAt(now - this.startedAt), cols, rows),
      now,
      cols,
      rows,
    );
    const lines = cellsToAnsi(frame, {color: true, background: false})
      .replace(/\n$/, '')
      .split('\n');
    if (this.abortHint && now <= this.abortHintUntil && lines.length > 0) {
      const hint = `\u001B[1;38;2;255;223;120m${centerText(this.abortHint, cols)}\u001B[0m`;
