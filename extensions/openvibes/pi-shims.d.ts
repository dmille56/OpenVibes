...
declare module "@mariozechner/pi-coding-agent" {
	...
	export interface ExtensionContext {
		...
		ui: {
			...
			getEditorComponent?(): ((tui: TUI, theme: EditorTheme, keybindings: TUIKeybindingsManager) => CustomEditor) | undefined;
		};
		sessionManager: {
	...
	export interface ExtensionAPI {
		...
		on(event: "context", handler: (event: { messages: any[] }) => void | Promise<{ messages: any[] } | void>): void;
		on(event: "session_start" | "agent_start" | "agent_end" | "session_shutdown", handler: (event: unknown, ctx: ExtensionContext) => void | Promise<void>): void;
		on(event: "message_start" | "message_update" | "message_end", handler: (event: { message: { role: string; content: unknown } }, ctx: ExtensionContext) => void | Promise<void>): void;
		on(event: "tool_execution_start" | "tool_execution_end", handler: (event: { tool?: { name?: string } }, ctx: ExtensionContext) => void | Promise<void>): void;
		on(event: string, handler: (...args: any[]) => void | Promise<void>): void;
	...
	export class CustomEditor implements Component {
		...
		constructor(tui: TUI, theme: EditorTheme, keybindings: TUIKeybindingsManager, options?: { paddingX?: number; paddingY?: number });
		handleInput(data: string): void;
		render(width: number): string[];
		invalidate(): void;
		getText(): string;
